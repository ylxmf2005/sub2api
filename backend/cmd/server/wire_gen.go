...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	settlementPoolRepository := repository.NewSettlementPoolRepository(client, db)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, settlementPoolRepository)
	...
	monitorHandler := handler.NewMonitorHandler(dashboardService, opsService)
	redeemHandler := handler.NewRedeemHandler(redeemService)
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService)
	settlementPoolService := service.NewSettlementPoolService(settlementPoolRepository, groupRepository, apiKeyAuthCacheInvalidator)
	settlementPoolHandler := handler.NewSettlementPoolHandler(settlementPoolService)
	...
	groupHandler := admin.NewGroupHandler(adminService, dashboardService, groupCapacityService, settlementPoolService)
	...
	adminSettlementPoolHandler := admin.NewSettlementPoolHandler(settlementPoolService)
	...
	resourceSupplyRepository := repository.NewResourceSupplyRepository(client, db)
	resourceSupplyService := service.NewResourceSupplyService(resourceSupplyRepository, accountRepository, groupRepository, settingService, accountTestService, openAIOAuthService, apiKeyAuthCacheInvalidator, billingCacheService)
	resourceSupplyHandler := admin.NewResourceSupplyHandler(resourceSupplyService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminSettlementPoolHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, resourceSupplyHandler)
	...
	ccgoDeviceLoginUserReader := service.ProvideCcgoDeviceLoginUserReader(userRepository)
	ccgoDeviceLoginTokenIssuer := service.ProvideCcgoDeviceLoginTokenIssuer(authService)
	ccgoService := service.NewCcgoService(ccgoRepository, connectionManager, manager, ccgoDeviceLoginUserReader, ccgoDeviceLoginTokenIssuer)
	ccgoHandler := handler.NewCcgoHandler(ccgoService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, monitorHandler, redeemHandler, subscriptionHandler, settlementPoolHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, handlerResourceSupplyHandler, ccgoHandler, idempotencyCoordinator, idempotencyCleanupService)
